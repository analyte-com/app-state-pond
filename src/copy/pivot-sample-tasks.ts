...
import { buildPivotedSampleTasksParquetSchema, taskColumnName, parquetValue } from "./pivot-utils";
...
export async function pivotToParquet(
  ...
  try {
    ...
    // we use a MAX lower number here because we have ~5 tasks per sample
    const MAX_ROWS = 10000; 
    
    // we need the Pivot schema constructed from the TaskTrees tables
    parquetSchema = await buildPivotedSampleTasksParquetSchema(pond);
...
async function queryMinMax(
  ...
  const rs = await safeQuery(pond, `
    SELECT
      500000 as min, max(sampleId) as max 
    FROM VSAMPLE_TASKS 
  `);
...
async function queryChunkedByIds(
  ...
  let rs = await safeQuery(pond, `
    SELECT
      sampleId, taskTreeId, valueTypeCode,
      id,
      stateCode, state,
      value, valueOffDetection, valueStateCode, valueState,
      doneUtc, doneById, doneBy, 
      instrumentId, instrument, intrumentStateCode, instrumentState,
      modifiedCode, modified, 
	    repeatedCode, repeated, repetitionNum, replicationNum,
      notes
    FROM VSAMPLE_TASKS 
  ...
  logger.elapsed(`queryChunkedByIds: count= ${rs.result.rows.length}`);
...
async function pivotChunkToParquet(
  ...
  rows: any[]
    ...
    logger.info(`pivotChunkToParquet: file= ${chunkFileName}`);
    ...
    for (let j=0; j < rows.length; j++) {
      ...
      if (current !== null && current.sampleId !== sampleId) {
        //logger.elapsed(`changing current ${sampleId} ${id}`);
        // it has changed
        // first push the current to pivoted parquet file 
        //process.stdout.write(`\rWrite count=${count} sampleId=${sampleId} taskTid=${taskTreeId}`);
      ...
      t.doneUtc = parquetValue('DATETIME', new Date(t.doneUtc));
      ...
      current[colName] = t ;
      //logger.elapsed(`add to current current ${sampleId} ${id}`);
    };
